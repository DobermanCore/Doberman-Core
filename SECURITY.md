...
Every [GitHub Release](https://github.com/DobermanCore/Doberman-Core/releases) ships a CycloneDX SBOM
(`sbom.json`) listing the exact resolved dependency set for that version, generated by `pip-audit`
straight from `pyproject.toml`. Use it to check whether a disclosed vulnerability in one of
Doberman's dependencies affects the version you're running. See [RELEASING.md](docs/RELEASING.md#software-bill-of-materials-sbom)
for how it's built, and to generate one yourself.
