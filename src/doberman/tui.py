...
def _row_cells(row: dict, widths: dict[str, int] | None = None) -> tuple[Text, ...]:
    ...
    return (
        ...
        Text(_truncate(render.humanize_auth_result(row.get("auth_result")), widths["auth"])),
...
class WhyScreen(ModalScreen[None]):
    ...
    BINDINGS = [
        ...
        Binding("b", "jump_block_next", "next BLOCK", show=False),
        Binding("B", "jump_block_prev", "prev BLOCK", show=False),
        Binding("a", "jump_auth_next", "next AUTH", show=False),
    ]
...
class HelpScreen(Screen[None]):
    ...
    _TEXT = (
        ...
        "r            reload\n"
        "?            this help screen\n"
        "/            filter rows (substring match on verdict, target, action, reason codes)\n"
        "escape       clear the filter (while it has focus), or close this screen / the why screen\n"
...
class DecisionExplainerApp(App[None]):
    ...
    def check_action(self, action: str, parameters: tuple[object, ...]) -> bool | None:
        # `escape`/"clear" is only meaningful (and only shown) while the
        # filter input actually has focus - otherwise it's a footer entry
        # that does nothing when pressed, the exact "trap" the design
        # critique flagged. False (not None) hides it outright rather than
        # merely graying it out.
        if action == "clear_filter":
            try:
                return self.focused is self.query_one("#filter", Input)
    ...
    def _update_subtitle(self) -> None:
        ...
        self.sub_title = (
            f"showing {len(self._visible_rows)} of {len(self._rows)} - {self._repo_root}"
        )
    ...
    def _with_next_step(self, body: str, row: dict) -> str:
        next_line = _next_step_line(row.get("final_verdict"))
        return f"{body}\n\n{next_line}" if next_line else body
    ...
    def _show_explanation(self, index: int) -> None:
        ...
        if cached is not None:
            self._set_panel(self._with_next_step(cached, row))
            return
        ...
        if not llm_enrichment_enabled():
            ...
            self._set_panel(self._with_next_step(text, row))
            return
    ...
    def _explain_worker(self, row: dict) -> None:
        ...
        def _apply() -> None:
            ...
            if self._current_key == key:
                self._set_panel(self._with_next_step(full, row))
    ...
    def full_why_text(self, row: dict) -> str:
        """The complete why text for `row`: cached body, every reason code, the
        action id, and a "Next" step - what `WhyScreen` displays."""
        key = _row_key(row)
        cached = self._explain_cache.get(key)
