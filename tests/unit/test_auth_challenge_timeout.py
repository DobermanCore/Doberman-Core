...
def test_an_unanswered_challenge_expires_within_ten_minutes():
    ...
    Before this pin the chain summed to 510s per pass (elicitation alone waited 300s), so a
    ``two_factor`` challenge nobody answered stayed approvable for ~13.5 minutes while the
    README promised a 2-minute dialog. Both halves are pinned: the ceiling itself, and that
    two full passes of every built-in channel finish before it, so the ceiling only ever
    bites on silence and never preempts a human mid-answer.
    """
    from doberman.auth.dashboard_prompter import DEFAULT_TIMEOUT_S as DASH_S
    from doberman.auth.elicitation_prompter import ANSWER_TIMEOUT_S as ELICIT_S
