...
enrolled 2FA (two-factor authentication), otherwise the local Doberman password set with `doberman
password set`. With neither enrolled, the action fails closed (denies by default rather than
...
> `doberman: command not found`. If you already hit this, reinstall `doberman-core`. The existing
> entries start working again. Then run the global uninstall below. `doberman doctor` confirms the
> entries resolve (its `Hook command` line) and fails, naming the fix, when `doberman` is not on PATH
> for the host.
