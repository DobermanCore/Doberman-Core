...
| `doberman tui` | Interactive decision log with a plain-language "why" panel. Keys: `?` help (list every binding; a proper modal - it never stacks and other keys are inert while it's open), `/` filter, `b`/`B`/`a` next/prev BLOCK or next AUTH, `w`/`enter` full-screen why, `tab` switch focus (the focused pane's border also gets a `[focus]` title, not just a color change), `y` copy id, `home`/`end`, `r` reload, `q` quit. The footer drops the least-used bindings (`B`/`a`/`y`/`r`) below 100 columns - they still work, `?` still lists them. Below 60x12 the app shows a one-line "resize" notice instead of the browser. Needs the `tui` extra. | `--path`/`-p`, `--last`/`-n` |
