...
tool call an agent makes and decides `PASS`/`AUTH`/`BLOCK` before it executes, but it does not train,
...
`partial` means defense-in-depth, never airtight — the same wording the [README](../README.md) uses
for its own secret detection: no single rule is a guarantee, and the "Honest limit" column names the
specific gap. `covered` means the mechanism exists and is tested, not that the risk category is fully
closed — read the limit column for every row regardless of status. Nothing in this document is a
compliance attestation; it is an engineering summary of what ships today, grounded in the
[`ReasonCode`](REASON_CODES.md) enum and the rule/detector modules that raise each code.
...
| LLM08:2025 | Vector and Embedding Weaknesses | `not addressed` | — | Doberman has no visibility into embedding stores, RAG retrieval, or vector-database access — out of its layer entirely. |
| LLM09:2025 | Misinformation | `not addressed` | — | Doberman does not evaluate the truthfulness of model output. The shadow adjudicator (`engine/adjudicator.py`) is advisory-only and judges decision *risk*, never factual accuracy, and has no production caller today ([Authority tiers](AUTHORITY_TIERS.md) T3). |
| LLM10:2025 | Unbounded Consumption | `not addressed` | — | Doberman does not meter or rate-limit LLM inference calls, tokens, or API cost. `bulk_operation` and `anomalous_egress_velocity` bound destructive blast-radius and egress-connection bursts respectively — a different concern, not model/API resource consumption. |
...
| GOVERN 1.2 | `partial` | Doberman's own prime directives (fail closed, raise-only, no secrets) are documented and CI-enforced against itself. | Governs Doberman's own build, not the deploying organization's broader AI risk policy — out of scope for a point tool. |
| MEASURE 2.6 | `covered` | The benchmark harness runs the real decision engine over a labeled detection corpus and CI gates on any regression in attack-bypass rate ([Benchmarks](BENCHMARKS.md)). | Corpus coverage is uneven — [Authority tiers](AUTHORITY_TIERS.md) documents a gap in benign delete-command false-positive coverage specifically. |
