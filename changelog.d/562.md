...
- Command-shaped benchmark actions now classify their egress destination through the same extractor the
  proxy uses in production (`doberman.proxy.normalize._extract_egress_destination`), never overriding an
  adapter-supplied destination, so the harness measures shipped command-egress behavior instead of
  under-reporting it; see `docs/BENCHMARKS.md`'s "Harness parity with the proxy" note for the moved
  RedCode numbers.
