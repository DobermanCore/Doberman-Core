...
- **Stylometric co-occurrence gate.** A per-entity **prompt-style baseline** (coarse scalar buckets only — length, word shape, punctuation/case/digit density; never any text) scores each turn's style as an empirical-CDF p-value, exactly the SL4 calibration idea applied to prompts. The gate steps up **only** when an *extreme* style outlier co-occurs with a *sensitive* apparent intent (credential / destructive / external-send) — **never on style alone** (people type differently tired/mobile/pasting; a device or language switch is drift, not an attack). Style-weird alone is **tagged, not gated**. Cold-start rule: stylometric gating stays inert until the prompt baseline matures (the same maturity rule as the SL baselines); Tier 0 is active from turn one. Known limitation: shared accounts blend typists, degrading the style signal toward noise — co-occurrence bounds the cost at one extra tap.
- **Tag-and-pass: turn signals feed the action stage (raise-only).** Sub-threshold observations do something: every *released* turn publishes its stylometric p-value and heuristic flags as a per-entity `TurnContext`. The action stage consumes it two ways — the SL4 surprise term accepts a **bounded, non-negative** contribution (a flagged turn makes its follow-on actions score harsher; a clean turn contributes exactly nothing), and actions tracing to **flagged pasted segments** inherit `provenance: untrusted_data` (formalizing the SL2.2 source), which makes the lethal-trifecta floor reachable for them. Raise-only by construction; a blocked turn publishes nothing.
...
### `v0.13.0` — Turn Gate (pre-inference chokepoint) — _Unreleased (in review)_

A second invocation point for the decision engine, consulted at a host pre-inference hook so a flagrant turn is judged before any inference token is spent. Additive — no change to the action path; the engine stays pure (the `turngate/` adapter injects the tier guardrails). See the Feature 11 section above.

- **TG1.1** — `TurnObject` (frozen, redaction-safe — HMAC fingerprints + coarse features + origin-tagged segments; `SegmentOrigin`, `ApparentIntent`).
- **TG1.3** — `decide_turn` (Tier 0 short-circuit, Tier 1 BLOCK→AUTH clamp, AUTH-first on error) reusing `combine` + the `Decision` model.
- **TG2.1 / TG2.2** — Tier 0 signatures (`instruction_nullification`, `authority_override`, `secret_export`, `encoded_payload`) with the issue-vs-mention discriminator and origin rule (untrusted → BLOCK + `indirect_injection`; typed+issued → BLOCK; typed+mentioned → AUTH; fences don't exempt).
- **TG3.1** — Tier 1 heuristic step-up classes (AUTH-only, structurally BLOCK-incapable).
- **TG4.1 / TG4.2** — repeat-after-block escape hatch (per-entity TTL'd LRU fingerprint cache; tiered, restated challenge — Tier 0 → 2FA; single-use; lockout on the third attempt).
- **TG1.2** — `normalize_turn`, redacted turn logging (`action_type='turn'` in the existing `decisions` table), and the `gate_turn` hook seam (disabled mode via `DOBERMAN_TURN_GATE`; fail-toward-human).
- **TG3.2** — stylometric co-occurrence gate: a per-entity prompt-style baseline (scalar buckets in the existing SL4 stores — no schema migration) scored as an empirical-CDF p-value; steps up only on extreme-outlier × sensitive-intent co-occurrence, tags otherwise; inert until the baseline matures (the SL maturity rule); released turns only teach it.
- **TG3.3** — tag-and-pass handoff: released turns publish `TurnContext{style_pvalue, flags}`; the SL4 surprise term gains a bounded, non-negative (raise-only) contribution, and actions tracing to flagged pasted segments inherit `provenance: untrusted_data` (the SL2.2 formalization).
- **Boundary** — the import-linter contract now forbids the policy core from importing **either** invocation adapter (`doberman.proxy`, `doberman.turngate`).
- **Deferred:** Feature CB cost observability (core seam + loop detector) and budget ceilings (platform).
