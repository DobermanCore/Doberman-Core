...
`doberman hook post` runs *after* a tool executes: it scans the tool's **output** for credential-like material — so a `Read`/`Bash`/MCP call that *returns* a secret is **blocked from reaching the model** (the secret is never echoed) — and records each call — plus a sticky per-session **taint** marker when a secret enters context — in a local, redacted decision history. That taint powers a **multi-step exfiltration floor**: the *pre*-hook raises an egress (web/network/MCP) in a session that has already accessed a secret — `ask` (light/balanced) or a hard `deny` (strict/paranoid) — catching read-secret-then-send-it exfil that no single-call rule can see. And when an outbound value *exactly* matches (by keyed-HMAC fingerprint) a secret that entered the session earlier, that **confirmed** read-then-send is a hard `deny` in **every** mode — even `light`. Both handlers **fail closed** and are import-light, so they add minimal latency to each call.
...
- **Bare-token fixture/pattern-text suppression is WEAK-path only.** A bare (non-assignment) token that is regex-pattern source text being quoted (e.g. `sk-ant-[A-Za-z0-9_-]{20,}`) or carries an obvious fixture marker (EXAMPLE/SAMPLE/FAKE/DUMMY, or a long ascending digit run) is not stepped up by the high-entropy heuristic alone (#73). This deliberately does **not** apply to a known credential-shape match, which can still drive `secret_exfiltration` — a marker alone can't be trusted to silence that stronger, BLOCK-capable signal (raise-only forbids weakening it), so a fixture-marked value with a real credential prefix is still flagged.
